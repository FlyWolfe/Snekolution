# -*- coding: utf-8 -*-
"""
Created on Mon Apr 23 11:13:58 2018
...
class Snake():
    def __init__(self, x, y, size, score, maxLength):
        self.x = x
        self.y = y
        self.size = size
        self.score = score
        if x > Cons.BOARD_WIDTH / 2:
            xDir = 1
        else:
            xDir = 0
        if y > Cons.BOARD_HEIGHT / 2:
            yDir = 1
        else:
            yDir = 0
    ...
    def loadImages(self):
        ...
        try:
            self.body = ImageTk.PhotoImage(bodyImage)
            self.head = ImageTk.PhotoImage(headImage)
